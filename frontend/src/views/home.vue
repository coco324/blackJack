...
<template>
  <div class="min-h-screen bg-cover bg-center flex items-center justify-center" :style="{ backgroundImage: `url(${backgroundImage})` }">
    ...
    <div v-if="UserStore().isLogin" class="absolute top-2 left-0 flex gap-4 p-4">
      <button v-if="UserStore().user.getIsAdmin()" class="p-4 rounded-lg bg-red-500 hover:bg-red-600" @click="router.push('/admin')">Page admin</button>
      <button class="p-4 rounded-lg bg-blue-500 hover:bg-blue-600" @click="router.push('/exam')">Page exam</button>
    </div>
    <div v-if="UserStore().isLogin" class="absolute top-6 right-6 flex gap-4 p-2 ">
